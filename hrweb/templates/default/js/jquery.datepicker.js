        ...
        function()
        {
          ...
          if (!alreadyExists && s.createButton) {
            ...
            var obj_fix = {
                ...
                height: parseInt($this.css("margin-top")) || 0
              },
              button_size = {
                width: 18,
                height: 15
              };
            controller.button = $('<sup class="dp-choose-date" title="' + $.dpText.TEXT_CHOOSE_DATE + '"></sup>').bind('click', function(){
              $this.dpDisplay();
              this.blur();
              return false;
            }).css({
              ...
              "position": "absolute",
              "z-index": $this.attr("data-index") || 5,
              "left": $this.position().left + $this.innerWidth() + obj_fix.width - button_size.width - 3,
              "top": $this.position().top + ($this.innerHeight() - button_size.height) / 2 + obj_fix.height
            });
            //$pop.css('top', eleOffset.top + $ele.innerHeight() + c.verticalOffset);
 ...
 **/
    ...
    dpGetDayCell : function(d)
    {
      var c = _getController(this[0]);
      if (c) {
        return c.getDayCell(d);
      }
      return null;
    },
    
    dpGetDisplayedMonth : function(){
      var c = _getController(this[0]);
      if (c) {
        return c.getDisplayedMonth();
      }
      return null;
    },
    
    dpGetDisplayedYear : function(){
      var c = _getController(this[0]);
      if (c) {
        return c.getDisplayedYear();
      }
      return null;
    },
 ...
 **/
  ...
  $.extend(
    ...
    { 
      ...
      getDayCell : function(d){
        var selectorString = 'td.current-month',
            $td = null;
...
//                $('<a href="#" class="selecteee">aaa</a>'),
        ...
        if (!c.inline) {
          ...
          if (this.verticalPosition == $.dpConst.POS_TOP) {
            $pop.css('top', eleOffset.top + $ele.innerHeight() + c.verticalOffset);
          }
