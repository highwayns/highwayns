...
namespace HPSRESUME
{
    public partial class FormSchool : Form
    {
        ...
        private DB db;
        public FormSchool(DB db, string resumeid)
        {
            this.db = db;
        ...
        private void init()
        {
            ...
            if (db.GetSchool(0, 0, "id,[StartDate],[EndDate],[School],[Special] ,[Graduation]", "EmpId="+resumeid, "", ref ds))
            {
        ...
        {
            int id = 0;
            String fieldlist = "EmpId,[StartDate],[EndDate],[School],[Special] ,[Graduation],UserId";
            String valuelist = "" + resumeid + ",'"
                + txtStartDate.Text + "','" + txtEndDate.Text + "','" + txtSchool.Text
                + "','" + txtSpecial.Text + "','" + txtGraduation.Text + "','"
                + db.db.UserID + "'";
            if (db.SetSchool(0, 0, fieldlist,
                                 "", valuelist, out id))
            {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                int id = 0;
                String wheresql = "id=" + txtId.Text;
                if (db.SetSchool(0, 2, "", wheresql, "", out id) && id == 2)
                {
        ...
        private void dataGridView1_SelectionChanged(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                txtSchool.Text = dataGridView1.SelectedRows[0].Cells[3].Value.ToString();
                txtSpecial.Text = dataGridView1.SelectedRows[0].Cells[4].Value.ToString();
                txtGraduation.Text = dataGridView1.SelectedRows[0].Cells[5].Value.ToString();
            }
