...
static ConnMap_sp_t g_push_server_conn_map;
static shared_ptr<CPushServConn> g_master_push_conn;

static CServInfo<CPushServConn>* g_push_server_list;
...
// 	serv_init<CPushServConn>(g_push_server_list, g_push_server_count);
	CServInfo<CPushServConn>::Init(g_push_server_list, g_push_server_count);
...
shared_ptr<CPushServConn> get_push_serv_conn()
{
    // CPushServConn* push_conn = NULL;
    shared_ptr<CPushServConn> push_conn;
...
void CPushServConn::Connect(const char* server_ip, uint16_t server_port, uint32_t serv_idx)
{
	...
	m_serv_idx = serv_idx;
	m_handle = netlib_connect(server_ip, server_port, imconn_callback_sp, (void*)&g_push_server_conn_map);
    
	if (m_handle != NETLIB_INVALID_HANDLE) {
		g_push_server_conn_map.insert(make_pair(m_handle, shared_from_this()));
	}
}
...
// 	serv_reset<CPushServConn>(g_push_server_list, g_push_server_count, m_serv_idx);
    CServInfo<CPushServConn>::Reset(g_push_server_list, g_push_server_count, m_serv_idx);
...
void CPushServConn::OnConfirm()
{
    ...
    g_master_push_conn = static_pointer_cast<CPushServConn>(shared_from_this());
}
