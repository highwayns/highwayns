...
{
    public partial class FormAnswerQuestionB : Form
    {
        private int testId = 0;//考试编号
        private int num;//试题编号
        public string answer;//当前正确答案
        UserInfor[] userInfors;//用户信息
        private static NC.HEDAS.Lib.CmWinServiceAPI db = new NC.HEDAS.Lib.CmWinServiceAPI();
        private const int Max_User_Count = 10;//最大用户数
        private const int Max_Wait_Time = 20;//最长等待时间
        private int index = 0;//当前答题者索引
        private int count = 0;//等待时间
        /// <summary>
        /// home page2
        /// </summary>
        private string home2 = null;
        public FormAnswerQuestionB(int testId)
        {
            this.testId = testId;
            InitializeComponent();
            getTestUser();
        ...
        private void setTestInfo(int testId, int questionId, int questionType, string real_answer)
        {
            ...
            {
                userInfors[index].setTestInfo(testId, questionId, questionType, real_answer);
            }
        }
        /// <summary>
        /// 退出
        ...
        private void getFirst()
        {
            string sql = "select top 1 a.*,b.QuestionType from tbl_Question_Single a,tbl_Test_Detail b where a.ID = b.QuestionId and b.testId="
                + testId.ToString() + " and b.QuestionType=1 order by a.ID";
            DataSet ds = db.ReturnDataSet(sql);
        ...
        {
            string sql = "select top 1 a.*,b.QuestionType from tbl_Question_Single a,tbl_Test_Detail b where a.ID = b.QuestionId and b.testId="
                + testId.ToString() + " and b.QuestionType=1 order by a.ID desc";
            DataSet ds = db.ReturnDataSet(sql);
            if (ds.Tables[0].Rows.Count == 1)
            {
                clear();
        ...
        {
            string sql = "select top 1 a.*,b.QuestionType from tbl_Question_Single a,tbl_Test_Detail b where a.ID = b.QuestionId and b.testId="
                + testId.ToString() + " and a.ID>" + num.ToString() + " and b.QuestionType=1 order by a.ID";
            DataSet ds = db.ReturnDataSet(sql);
            if (ds.Tables[0].Rows.Count == 1)
            {
                clear();
        ...
        private void getPrev()
        {
            if (num <= 1) return;
            string sql = "select top 1 a.*,b.QuestionType from tbl_Question_Single a,tbl_Test_Detail b where a.ID = b.QuestionId and b.testId="
                + testId.ToString() + " and a.ID<" + num.ToString() + " and b.QuestionType=1 order by a.ID desc";
            DataSet ds = db.ReturnDataSet(sql);
            if (ds.Tables[0].Rows.Count == 1)
            {
                clear();
        ...
        private void setScore()
        {
            ...
            if (userInfors[index].isSelected)
            {
                ...
                else
                {
                    ...
                    updateRank();
                    userInfors[userInfors.Length - 1].Clear();
                }
                getNext();
            }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void FormAnswerQuestionB_FormClosed(object sender, FormClosedEventArgs e)
        {
        }
